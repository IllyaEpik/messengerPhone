...
interface FriendsContextType {
  ...
  moveUser: (user: IProfile | ProfileId, from: friendMenuVariant, to: friendMenuVariant) => void;
  clear: () => void
}
...
export function FriendsProvider (props: {children:ReactNode} ){
  ...
  // Инициализируем стейт списками id и профилей
    const [exlude, setExlude] = useState<FriendsExludeState>(initialFriendsExludeState)
    const [apper, setApper] = useState<FriendsApperState>(initialFriendsApperState)

    function moveUser(friend: IProfile | ProfileId, from: friendMenuVariant, to: friendMenuVariant) {
        if (from === "main" || from === to) return

        setExlude(prev => ({
            ...prev,
            [from]: prev[from].includes(friend.id) ? prev[from] : [...prev[from], friend.id]
        }))

        if ('avatar' in friend && to!=="main") {
            setApper(prev => ({
                ...prev,
                [to]: prev[to].some(item => item.id === friend.id) ? prev[to] : [...prev[to], friend]
            }))
        }
    }
    function clear() {
