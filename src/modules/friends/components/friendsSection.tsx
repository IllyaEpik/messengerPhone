		...
		<View style={styles.section}>
			...
			<View style={styles.headerSection}>
				...
				<Text
					...
					style={
						friends.length > 0 ? styles.activeText : styles.crayText
					}
				>
					Дивитись всі
				</Text>
			...
			{(() => {
				const displayedFriends = [
					...friends,
					...apper[variant].filter(
						(added) =>
							!friends.some(
								(existing) => existing.id === added.id,
							),
					),
				];
				...
				return displayedFriends.length > 0 ? (
					displayedFriends.map((friend, index) => {
						const isExcluded =
							exlude[variant]?.includes(friend.id) ||
							(index > 2 && !props.isSelected);
					...
					<Text style={styles.crayText}>
						{props.noFriendsMessage}
					</Text>
				);
			})()}
		</View>
