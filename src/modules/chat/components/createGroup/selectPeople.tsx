	...
	useEffect(() => {
		...
		if (friends) {
			const grouped = friends.reduce(
				(acc: Record<string, Contact[]>, friend) => {
	...
	if (isNext) {
		...
		return (
			<CreateGroupDetails
				...
				initialParticipants={selectedParticipants.map((c) => ({
					id: c.id,
					name: c.name,
					avatar: c.avatar,
				}))}
			...
			<View style={styles.layoutContainer}>
				...
				<View style={styles.mainContainer}>
					...
					<Text style={styles.mainTitle}>
						{isEdit ? "Додати учасника" : "Нова група"}
					</Text>
					...
					<SectionList
						...
						renderSectionHeader={({ section: { title } }) => (
							<Text style={styles.groupSectionHeader}>
								{title}
							</Text>
						)}
						renderItem={({ item }) => {
