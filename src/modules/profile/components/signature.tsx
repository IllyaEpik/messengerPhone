...
export function Signature(props: IProps) {
	const { setScrollEnabled, setSignature, user, edit, signature } = props;
					...
					<View style={styles.buttons}>
						<Pressable
							style={[
								styles.color,
								styles.brown,
								color === "#543C52" && styles.selectedColor,
							]}
						...
						<Pressable
							style={[
								styles.color,
								color === "#070A1C" && styles.selectedColor,
							]}
					...
					{(signature || user?.profile?.signature) && (
						<Image
							...
							source={{
								uri:
									signature ||
									`http://10.0.2.2:8000/media/crackedAvatars/${user?.profile?.signature}.jpg`,
							}}
							resizeMode="contain"
						/>
					)}
